        ...
        for (int j = 0; j < height; j++)
        {
            ...
            for (int k = -1; k < 2; k++)
            {
                for (int l = -1; l < 2; l++)
                {
                    if (i + k > 0 && i + k < width)
                    {
                        red += image[i+k][j].rgbtRed;
                        green += image[i+k][j].rgbtGreen;
                        blue += image[i+k][j].rgbtBlue;
                        denominator++;
                    }
                    if (j + l > 0 && j + l < height)
                    {
                        red += image[i][j+l].rgbtRed;
                        green += image[i][j+l].rgbtGreen;
                        blue += image[i][j+l].rgbtBlue;
                        denominator++;
                    }
                }
            }
